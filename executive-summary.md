...
- **For a business user, standard MCP isn't a friction *reducer* — it's the *on-ramp*.** There's no CLI to save steps over; MCP is the only practical path to live Salesforce data. The comparison becomes **"MCP vs. nothing / manual CSV exports,"** not "MCP vs. hand-written SOQL."
